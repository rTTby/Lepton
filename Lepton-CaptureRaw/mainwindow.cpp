...
MainWindow::MainWindow(QWidget *parent)
    ...
    layout->addWidget(imageLabel, 0, 1, Qt::AlignCenter);

    QPixmap filler(ImageWidth, ImageHeight); filler.fill(Qt::red);
    imageLabel->setPixmap(filler);
    
    maxTemp = new QLabel();
    layout->addWidget(maxTemp, 2, 2, Qt::AlignCenter);
    
    minTemp = new QLabel();
    layout->addWidget(minTemp, 2, 0, Qt::AlignCenter);
    
    displayAlert = new QLabel();
    layout->addWidget(displayAlert, 3, 1, Qt::AlignCenter);
    ...
    layout->addWidget(snapshotButton, 1, 1, Qt::AlignCenter);
    connect(snapshotButton, SIGNAL(clicked()), this, SLOT(saveSnapshot()));
    
    //QPushButton *button1 = new QPushButton("refresh", mainWidget);
    //layout->addWidget(snapshotButton, 1, 1, Qt::AlignCenter);
    //button1->setGeometry(500, 400, 100, 30);
    //connect(button1, SIGNAL(clicked()), this, SLOT(performFFC()));
...
void MainWindow::updateImage(unsigned short *data, int minValue, int maxValue){
    ...
    maxTemp->setText(QString("Max Value Temp: %1 °C ").arg((maxValue/100.0) - 272.15, 0, 'f', 2));
    minTemp->setText(QString("Min Value Temp: %1 °C ").arg((minValue/100.0) - 275.15, 0, 'f', 2));
    
    currentTemp = (maxValue/100.0) - 272.15;
    
    if (currentTemp >= 50.0) {
        displayAlert->setText(QString("Medical&Safety Alarm - Detect Hot Temperature !Please Check Your Surroundings!"));
        //rgbImage.setPixel(x, y, qRgb(colormap[3*scaledValue], colormap[3*scaledValue+1], colormap[3*scaledValue+2]));
    }  else {
        displayAlert->setText(QString("Medical&Safety Alarm - Detect Safe Temperature"));
    }
    
    // while (currentTemp < 40.0) {
    //     if (currentTemp >= 40.0) {
        
    //         displayAlert->setText(QString("Gerrald Detect Hot Temperature !Please Check Your Surroundings!"));
    //         system("curl -X POST -H 'Authorization: Bearer PEB5hwzaTwO2UPUthiXWradXXlZBMX17vk8YpHVDqFH' -F 'message=Gerrald Detect Hot Object !Please Check Your Surroundings!' https://notify-api.line.me/api/notify");        
    //    }else {
    //        displayAlert->setText(QString("Gerrald Detect Safe Temperature"));
    //    }
    //}
